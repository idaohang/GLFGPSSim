/* sd_mgw.c -- MinGW32/Windows-specific module for:
...
/* ONLY if Windows + MinGW32 */
#include <windows.h>    /* allows H,S,R,A attribute set/reset in Windows */
...
int build_copy_name(char finalname[], 
  ...
  {
   char subpathname[MAXPATHNAMESIZE+1];    /* this is filled in below */
...
/* NOTE:  the TRUE return status of this function does NOT indicate whether
          ...
          (filefilter) var->ezattrib[0] = ' ' if matched, '!' if not.
*/
                                        ...
                                        const char filter[], char direcname[])
{
 ...
 /* Windows needs extra stuff to get info about DOS file attributes H,S,R,A */
 int slen;
 unsigned win_attr;
 char temp[MAXPATHNAMESIZE+1];
...
/* at this point patwork and newname->filtname have been scanned built the same way,
 ...
 /* NOTE: newname->ezattrib[0] is reserved for '!' char if pattern does NOT match */
 ...
 newname->modstamp = 0;
    ...
    /* assume MinGW32 Windows */
    /* Use Windows calls to get same file flags as TC code above -- don't apply
       to Linux, so don't set them in osattrib */

   /* temp must be set to complete file pathname */
   strcpy(temp,direcname);
   strcat(temp,newname->filenameonly);  

   /* DOS READONLY flag already implied by stat() with R but NO W */
   if (INVALID_FILE_ATTRIBUTES != (win_attr = GetFileAttributes(temp)))
     {
       if ((win_attr) & FILE_ATTRIBUTE_HIDDEN)  /* DOS HIDDEN flag */
         {
          newname->ezattrib[5] = 'H';
         }
       if ((win_attr) & FILE_ATTRIBUTE_SYSTEM)  /* DOS SYS flag */
         {
          newname->ezattrib[6] = 'S';
         }
       if ((win_attr) & FILE_ATTRIBUTE_ARCHIVE)  /* DOS ARCH flag */
         {
 ...
 if (0 == stat(olddirentry->name,&statbuf))
    ...
    */
    ...
    /* assume Windows/MinGW32 -- simulate EXEC flag same way as for DOS */
    if (!(newname->osattrib & S_IFDIR)) /* NOT a directory -- check if executable */
      {
...
static int ezattrib_chmod(const char ezattr[], const char filefullname[])
  {
   ...
   /* the following (H,S,R,A attributes) ONLY work under Windows, NOT Linux */    

   /* get the existing attributes so we don't change things we shouldn't... */
   if (INVALID_FILE_ATTRIBUTES != (allchg = GetFileAttributes(temp)))
     {
      /* mask off the H,S,R,A attributes using AND (bitwise complement) to reset */
      allchg &= (~(FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM | 
                  FILE_ATTRIBUTE_ARCHIVE | FILE_ATTRIBUTE_READONLY));
                  
      if (ezattr[5] == 'H')
        {
         allchg |= FILE_ATTRIBUTE_HIDDEN;           
        }
      if (ezattr[6] == 'S')
        {
         allchg |= FILE_ATTRIBUTE_SYSTEM;           
        }
      if (ezattr[7] == 'A')
        {
         allchg |= FILE_ATTRIBUTE_ARCHIVE;           
        }
      if (need_ro)
        {
         allchg |= FILE_ATTRIBUTE_READONLY;           
        }
   
      if (!SetFileAttributes(temp,allchg))
        {
         return FALSE;    
        }
     }   
          ...
          if (filtername(&filtdata,&singledir,pattern,dpath))
            {
             if (filtdata.ezattrib[1] != 'D') /* NOT directory flag */
               {
                if (filtdata.ezattrib[0] != '!') /* apply wildcard filter */
                  {
