/* winbgi3.cpp -- (Mingw32) libwinbgi3.a assumed for Windows
...
/* Borland graphics is available directly through Turbo C 2.01 GRAPHICS.LIB */
/* initgraph() is patched in "graphic2.h", otherwise same as <graphics.h> */

/* 
   For easy testing, copy Borland GRAPHICS.LIB to \prg\tc\clibrary\WINBGI2.LIB 
   to satisfy .PRJ linking requirements -- as of 12/15/2008 gdi32.lib and lm.lib 
   are automatically excluded from generated .PRJ file (32cvtdev2t.exe) --
   also copy all *.BGI and *.CHR files from TC compiler directory to same
   folder as executable binary so that distributed code will work -- GLF 
*/

#else
/* assume Windows -- compile as C++, compatible with C program callers */

/* Adapted from winbgi.cpp downloaded from internet -- uses C++ objects
   internally and is not easily converted to C only code -- therefore
   when building library for Windows, use winbgi3.cpp -- when building 
   library for X, either winbgi3.cpp or winbgi3.c should work.  In either
   case, the calling program may be either C or C++.
*/
...
/* Linux version uses X library and C code (tries to wrap C++ linkage in C externals) */
